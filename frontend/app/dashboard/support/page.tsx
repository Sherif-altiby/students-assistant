...
export default function SupportPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="font-display text-2xl font-extrabold text-ink">الدعم النفسي</h1>
        <p className="mt-1 text-sm text-ink/60">يحق لك حجز جلسة دعم نفسي واحدة كل شهر مع متخصص</p>
      </div>
      ...
      <Card>
        <form onSubmit={handleRequest} className="flex flex-col gap-4">
          <FieldShell label="ملاحظة قبل الجلسة (اختياري)">
            <textarea
              value={note}
          ...
          <Button type="submit" variant="danger" isLoading={isSubmitting} disabled={reachedLimit} className="self-start">
      ...
      <div className="flex flex-col gap-3">
        ...
        {isLoading ? (
          ...
          <Card className="text-center text-sm text-ink/50">لا توجد جلسات سابقة</Card>
        ) : (
          sessions.map((s) => (
            <Card key={s.id} className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-ink">{s.note || "بدون ملاحظات"}</p>
